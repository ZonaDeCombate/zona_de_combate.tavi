	...
	if (_isXLArsenal) then {
		...
		private ["_forceReplace", "_addActionText"];
			...
			if (_isTFR) then {
				...
				["AmmoboxInit",[_basebox, false, {(_target distance _this) < 3}, _addActionText, false, _forceReplace]] spawn XLA_fnc_arsenal;		
				
			} else {
				
				["AmmoboxInit",[_basebox, false, {(_target distance _this) < 3}, _addActionText, false]] spawn XLA_fnc_arsenal;			
			};
		...
		["AmmoboxInit", [_basebox, false, {(_target distance _this) < 3}]] spawn BIS_fnc_arsenal;
	};